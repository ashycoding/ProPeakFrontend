...
const TechIcon: React.FC<{ name: string }> = ({ name }) => {
  switch (name) {
    case 'React':
      return (
        <svg viewBox="-11.5 -10.23174 23 20.46348" className="w-5 h-5 fill-none stroke-current" strokeWidth="1.5">
    ...
    <section className="py-20 lg:py-24 bg-[#F0FDF8] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14 space-y-4">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#052E20] tracking-tight">
            Battle-Tested Technologies We Build With
          </h2>

          <p className="text-base sm:text-lg text-[#527A68]">
            We deploy modern, scalable languages, frameworks, and AI toolchains tailored to the needs of each project.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4">
          {techStackData.map((tech, idx) => (
            ...
            >
              ...
              <h4 className="text-sm font-bold text-[#052E20] mb-0.5">{tech.name}</h4>
              <p className="text-[11px] font-semibold text-[#10A87A]">{tech.category}</p>
            </motion.div>
          ))}
        </div>
